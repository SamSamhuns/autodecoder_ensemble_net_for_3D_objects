...
class PointRegDataset(Dataset):
    def __init__(
        ...
        if clas == 2:
            source = (np.loadtxt("./data/hand2.txt")).reshape(-1, 2)
            source = source[:256, :]
        if clas == 3:
            source = (np.loadtxt("./data/human_skeleton.txt")).reshape(-1, 2)
            source = source[:256, :]

        if clas == 4:
            source = (np.loadtxt("./data/skull_try_1.txt")).reshape(-1, 2)
            source = source[:256, :]
        ...
        for i in tqdm.tqdm(range(total_data)):
            ...
            targ = Tps_trans(
                np.tile(np.expand_dims(source, 0), [2, 1, 1]),
            ...
            if self.noise_ratio != 0:
                if noise_s:
                    source = source + np.random.normal(
                        0, self.noise_ratio, source.shape
                    )
                if noise_t:
            ...
            if self.outlier_ratio != 0:
                if outlier_s:
                    addi = np.asarray(
                        [
                            np.random.uniform(-2, 2, self.outlier_ratio),
                            np.random.uniform(-2, 2, self.outlier_ratio),
                        ]
                    ).T
            ...
            if self.missing_points != 0:
                if miss_source:
                    ...
                    selectingPoints = np.argsort(Pdist)[
                        np.random.choice(range(point_size))
                    ][self.missing_points :]
