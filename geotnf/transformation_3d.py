...
class GeometricTnf(object):
    ...
    def __call__(
        self, image_batch, theta_batch=None, padding_factor=1.0, crop_factor=1.0
    ):
...
class SynthPairTnf(object):
    ...
    def __init__(
        ...
        self.rescalingTnf = GeometricTnf(
            "affine", self.out_h, self.out_w, use_cuda=self.use_cuda
        )

        self.geometricTnf = GeometricTnf(
            geometric_model, self.out_h, self.out_w, use_cuda=self.use_cuda
        )
    ...
    def __call__(self, batch):
        ...
        cropped_image_batch = self.rescalingTnf(
            image_batch, None, self.padding_factor, self.crop_factor
        )  # Identity is used as no theta given
        # get transformed image
        warped_image_batch = self.geometricTnf(
            image_batch, theta_batch, self.padding_factor, self.crop_factor
        )  # Identity is used as no theta given
...
class TpsGridGen(Module):
    def __init__(
        ...
        self.grid_X, self.grid_Y, self.grid_Z = np.meshgrid(
            ...
            np.linspace(-1, 1, out_d),
        )

        # grid_X,grid_Y, grid_Z: size [1,H,W,D,1,1,1]
        self.grid_X = (
            torch.FloatTensor(self.grid_X).unsqueeze(0).unsqueeze(-1).unsqueeze(-1)
        )
        self.grid_Y = (
            torch.FloatTensor(self.grid_Y).unsqueeze(0).unsqueeze(-1).unsqueeze(-1)
        )
        self.grid_Z = (
            torch.FloatTensor(self.grid_Z).unsqueeze(0).unsqueeze(-1).unsqueeze(-1)
        )
        ...
        if use_regular_grid:
            ...
            self.Li = Variable(
                self.compute_L_inverse(P_X, P_Y, P_Z).unsqueeze(0), requires_grad=False
            )
    ...
    def forward(self, theta):
        warped_grid = self.apply_transformation(
            theta, torch.cat((self.grid_X, self.grid_Y, self.grid_Z), 4)
        )
    ...
    def compute_L_inverse(self, X, Y, ZZ):
        ...
        P_dist_squared = (
            ...
            + torch.pow(Zmat - Zmat.transpose(0, 1), 2)
        )
    ...
    def apply_transformation(self, theta, points):
        ...
        W_Z = torch.bmm(
            self.Li[:, : self.N, : self.N].expand((batch_size, self.N, self.N)), Q_Z
        )
        ...
        W_X = (
            ...
            .unsqueeze(5)
            .transpose(1, 5)
            .repeat(1, points_h, points_w, points_d, 1, 1)
        )
        W_Y = (
            W_Y.unsqueeze(3)
            .unsqueeze(4)
        ...
        A_X = torch.bmm(
            self.Li[:, self.N :, : self.N].expand((batch_size, 4, self.N)), Q_X
        )
        A_Y = torch.bmm(
            self.Li[:, self.N :, : self.N].expand((batch_size, 4, self.N)), Q_Y
        )
        A_Z = torch.bmm(
            self.Li[:, self.N :, : self.N].expand((batch_size, 4, self.N)), Q_Z
        )
        ...
        points_Z_for_summation = (
            points[:, :, :, :, 2]
            .unsqueeze(3)
            .unsqueeze(4)
            .expand(points[:, :, :, :, 2].size() + (1, self.N))
        )
        ...
        if points_b == 1:
            ...
            delta_X = points_X_for_summation.cuda() - P_X.expand_as(
                points_X_for_summation
            )
            delta_Y = points_Y_for_summation.cuda() - P_Y.expand_as(
                points_Y_for_summation
            )
            delta_Z = points_Z_for_summation.cuda() - P_Z.expand_as(
                points_Z_for_summation
            )

        dist_squared = (
            torch.pow(delta_X, 2) + torch.pow(delta_Y, 2) + torch.pow(delta_Z, 2)
        )
        # U: size [1,H,W,1,N]
        ...
        if points_b == 1:
            ...
            points_Z_batch = points_Z_batch.expand(
                (batch_size,) + points_Z_batch.size()[1:]
            )
