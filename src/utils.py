...
def get_X_y_from_npy(data_src, shuffle_seed=None):
    ...
    for path in Path(data_src).iterdir():
        ...
        if X is None and y is None:
            ...
            y = np.concatenate(
                [y, np.full((X_curr.shape[0], 1), label, dtype=np.int64)], axis=0
            )
        label += 1
...
def plot_roc_curve(
    fp_rate, tp_rate, title="Receiver operating characteristic", figsize=(8, 6)
):
    """
